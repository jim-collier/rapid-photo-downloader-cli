...
##		inclusive call buckets, per-subsystem attribution, and the caller chain of
##		the top leaves - by parsing the inferno/pprof SVG (its fg:w attribute = raw
##		sample counts). Runs two ways: plain (print the report, meant to run every
##		cicd run) and --check (print only when the newest flamegraph is newer than
##		the one last recorded in a local marker, then record it - meant for session
##		startup so a look is a no-op until there is something new to read).
...
##	Per-subsystem self-time buckets. Each is (label, [name-substrings]); a frame's
##	self-time lands in the FIRST bucket any of whose substrings appear anywhere in
##	its ancestor chain, else "other". Derived from design.md's hot paths (checksum,
##	metadata, transfer, scan/filter) - substrings are best-guesses until real Go
##	symbols exist, so retune the lists once the profile shows actual frame names.
BUCKETS = [
	("checksum / hashing ....", ["blake2b", "Blake2b", "checksum", "Checksum", "hash", "Hash"]),
	("metadata / exif .......", ["exif", "Exif", "metadata", "Metadata", "xmp", "Xmp", "DateTime", "exiftool"]),
	("file transfer / io ....", ["reflink", "io.Copy", "Copy", "syscall", "os.File", ".Read", ".Write", "sendfile"]),
	("scan / filter .........", ["filepath", "Walk", "glob", "Glob", "regexp", "Regexp", "include", "exclude", "scan", "Scan"]),
]
...
def fAnalyze(total, frames, top):
	...
	def bucketOf(anc):
		##	First bucket whose any substring appears in any ancestor name.
		for label, subs in BUCKETS:
			if any(s in a for a in anc for s in subs):
				return label
		return "other .................."

	selfBy, inclBy, byName = {}, {}, {}
	attribution = {label: 0.0 for label, _ in BUCKETS}
	attribution["other .................."] = 0.0
	for fr in frames:
		name = fr[0]
		...
		attribution[bucketOf(anc)] += s

	def pct(v):
		return f"{v / total * 100:5.1f}%"

	print("attribution (self-time by subsystem):")
	for label, _ in BUCKETS:
		print(f"  {label}: {pct(attribution[label])}")
	print(f"  other ..................: {pct(attribution['other ..................'])}")
	print()
...
##	History:
##		- 2026-07-09 JC: Created (adapted from the silkterm flame-report; buckets
##		  re-derived for the photo-CLI subsystems).
