...
##		vuln / security lines (go vet, govulncheck, gosec, plus the pipeline's own
##		WARNING: skips) so they can be addressed. Two modes: plain (print warnings
...
##	Distil warnings. Go vet says "vet:"/"warning"; govulncheck emits "Vulnerability"
##	+ GO-YYYY-NNNN ids; gosec emits "[gosec]"/"Severity:"/Gnnn rule ids; the pipeline
##	itself prints "WARNING:" when it skips a tool. Hard errors only appear in a failed
##	run's log (a passing run aborts on the first error). Drop the "0 warnings" noise.
warns="$(grep -inE 'warning|vet:|vulnerab|GO-[0-9]{4}-|\[gosec\]|severity:|\bG[0-9]{3}\b' "$log" 2>/dev/null \
	| grep -viE '0 warnings|no warnings|no vulnerabilities|found 0' || true)"
...
##	Script history:
##		- 20260709 JC: Created (adapted from the silkterm lint-report for Go tooling).
